...
class LocalOutfitRepository implements OutfitRepository {
  ...
  Stream<List<String>> watchClothingItemIds(String outfitId) {
    ...
    return query.watch().map((rows) => [for (final row in rows) row.clothingItemId]);
  }

  @override
  Stream<List<OutfitItem>> watchLinks() {
    return (_db.select(_db.outfitItems)
          ..orderBy([
  ...
  @override
  Future<void> save(OutfitsCompanion outfit, List<String> clothingItemIds) async {
    final id = outfit.id.value;
    await _db.transaction(() async {
      await _db.into(_db.outfits).insertOnConflictUpdate(outfit);
      await (_db.delete(_db.outfitItems)..where((t) => t.outfitId.equals(id))).go();
      final seen = <String>{};
      var order = 0;
      for (final clothingId in clothingItemIds) {
        if (!seen.add(clothingId)) continue;
        await _db.into(_db.outfitItems).insert(
  ...
  Future<void> clearCollage(String id) async {
    final existing = await getById(id);
    if (existing == null) return;
    final hasPhoto = existing.imagePath != null && existing.imagePath!.isNotEmpty;
    await (_db.update(_db.outfits)..where((t) => t.id.equals(id))).write(
      OutfitsCompanion(
        collageLayout: const Value(''),
        coverMode: Value(hasPhoto ? 'photo' : existing.coverMode),
        updatedAt: Value(DateTime.now()),
      ),
    );
  }
