...
import 'package:wardrobe/core/vision/garment_onnx.dart';
import 'package:wardrobe/core/vision/image_ops.dart';
import 'package:wardrobe/core/vision/model_assets.dart';
...
class TagOcr {
  ...
  Future<void> ensureLoaded() async {
    ...
    GarmentOnnx.loadSession('ocr_det', det.path);
    GarmentOnnx.loadSession('ocr_rec', rec.path);
    final detIo = GarmentOnnx.sessionIo('ocr_det');
    final recIo = GarmentOnnx.sessionIo('ocr_rec');
  ...
  void dispose() {
    if (!_loaded) return;
    GarmentOnnx.closeSession('ocr_det');
    GarmentOnnx.closeSession('ocr_rec');
    _loaded = false;
  }
  ...
  Future<List<PixelRect>> _detect(img.Image rgb) async {
    ...
    final out = GarmentOnnx.runSession(
      name: 'ocr_det',
